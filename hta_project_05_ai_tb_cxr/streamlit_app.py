...
# Page configuration
st.set_page_config(
    page_title="AI-Assisted TB Screening",
    page_icon="🤖",
    layout="wide",
...
def create_visualization(results):
    ...
    fig.update_layout(
        title='AI-Assisted TB Screening',
        xaxis_title='X Axis',
        yaxis_title='Y Axis',
        showlegend=True,
        height=500
...
def main():
    ...
    # Main header
    st.markdown('<h1 class="main-header">🤖 AI-Assisted TB Screening</h1>', unsafe_allow_html=True)
    st.markdown("**Cost-Effectiveness Analysis of Artificial Intelligence for Tuberculosis Diagnosis**")
    ...
    with st.sidebar:
        ...
        if isinstance(icer_value, str):
            st.metric("ICER", icer_value, "Cost-Effective")
        else:
            st.metric("ICER", f"₹{icer_value:,.0f}/QALY", "Cost-Effective")
    ...
    if page == "Overview":
        ...
        with col1:
            ...
            if isinstance(8500, str):
                st.metric("ICER", "8500", "Cost-Effective")
            else:
                icer_formatted = f"₹{config['icer']:,.0f}/QALY"
                st.metric("ICER", icer_formatted, "Cost-Effective")
